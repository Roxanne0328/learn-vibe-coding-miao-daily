...
手机点邮件链接会被浏览器/中间页弄丢凭证，所以用这条更稳的通道：

1. 打开登录页 → 填邮箱 → 点「✨ 发送魔法链接」
2. 去邮箱 → **长按**邮件里的按钮 → 「**拷贝链接地址**」
3. 回到登录页 → 粘进蓝色区域 → 点「🔗 用链接登录」→ 进主界面

> 原因和彻底解法见 [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md#坑-6--手机把--后的凭证弄丢终极-boss)
