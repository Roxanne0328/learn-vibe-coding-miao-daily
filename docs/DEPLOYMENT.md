...
| 问题 | 影响 | 状态 |
|------|------|------|
| 手机点链接仍会回登录页 | ~~需用粘贴通道绕过~~ | ✅ **已被 v1.1.3 验证码登录取代**——登录不再依赖点链接，手机电脑统一「填邮箱 → 收数字验证码 → 填码进主页」。粘贴通道折叠保留在登录页当兜底 |
| ~~云端数据同步在手机可能不灵~~ | ✅ **已解决（2026-09-12 02:20）** | 修复版云函数已部署（探针头 `X-Proxy-Version: 1.1.2-fix2` 可远程验证），手机端同步通道恢复 |
| 「确定访问」中间页 | 首次访问多点一次 | 等 ICP 备案通过 |

### v1.1.3 验证码登录要点

- **Supabase 邮件模板**（后台 Authentication → Email Templates → Magic Link）必须配置为含 `{{ .Token }}` 的文案，否则邮件里没有数字验证码
- **验证码长度实测 8 位**（官方文档写 6 位，别信）——前端校验已放宽 4~10 位
- 验证接口：`POST /auth/v1/verify`，`type` 用 `magiclink`（报 type 错时前端自动换 `email` 重试）
- 会话由前端本地写入（`miao_daily_session`），全程不碰 Supabase SDK
